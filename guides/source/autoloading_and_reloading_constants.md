...
Reloading can be enabled or disabled. The setting that controls this behavior is [`config.reload`][], which is `true` by default in `development` mode, and `false` by default in `production` mode. For backwards compatibility, Rails also supports `config.cache_classes`, which is equivalent to `!config.reload`.
...
In a Rails console there is no file watcher active regardless of the value of `config.reload`. This is because, normally, it would be confusing to have code reloaded in the middle of a console session. Similar to an individual request, you generally want a console session to be served by a consistent, non-changing set of application classes and modules.
